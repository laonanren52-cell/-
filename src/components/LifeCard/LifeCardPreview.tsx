...
export function LifeCardPreview({ card, compact = false }: { card: LifeCard; compact?: boolean }) {
  ...
  return (
    <Link
      to={`/cards/${card.id}`}
      className="group block h-full overflow-hidden rounded-3xl bg-white shadow-sm transition hover:shadow-glow"
    >
      <div className="relative h-48 overflow-hidden">
        ...
        <span className="absolute left-3 top-3 max-w-[60%] truncate rounded-full bg-white/90 px-3 py-1 text-xs font-semibold text-gray-900 shadow-sm">
          {card.category || "人生支线"}
        </span>
        <span className="absolute right-3 top-3 rounded-full bg-gray-900/80 px-3 py-1 text-xs font-semibold text-white shadow-sm">
          {imageLabel}
        </span>
      </div>
      <div className="space-y-3 p-5">
        <h3 className="line-clamp-2 text-lg font-bold leading-6 text-gray-900">{card.title}</h3>
        <p className="line-clamp-3 text-sm leading-6 text-gray-600">{summary}</p>
        <div className="flex flex-wrap gap-2">
          {visibleMoods.map((mood) => (
            <span key={mood} className="rounded-full bg-orange-50 px-3 py-1 text-xs font-semibold text-orange-700">
              {mood}
            </span>
          ))}
          {hiddenMoodCount ? (
            <span className="rounded-full bg-gray-100 px-3 py-1 text-xs font-semibold text-gray-500">+{hiddenMoodCount}</span>
          ) : null}
        </div>
