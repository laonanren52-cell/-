...
- 旧卡补图：默认卡或无图人生卡可在详情页点击「重新生成 AI 图片」补齐封面。
- 卡片预览：首页、轨迹页、复盘代表卡片统一使用上图下文结构，标题、文案、地点均截断显示，避免文字重叠。
...
1. 在线填写：部署后直接进入「设置 → 在线 AI API 配置」，填写 Base、Key、Model，点击「保存 API 配置」，系统会在配置完整时自动切换为 API 模式，再测试文本 API / 生图 API。
...
然后在设置页把 AI 模式切换为 `API 模式`。如果接口失败，系统会保存默认人生卡并写入 `aiImageError`，不会中断打卡流程。测试生图 API 成功后，设置页会直接展示返回图片。
...
- 如果打卡页勾选 AI 生图但仍处于 Mock 模式，页面会提示「继续保存默认卡片」或「去设置页开启 API 模式」，不会静默假装调用生图接口。
